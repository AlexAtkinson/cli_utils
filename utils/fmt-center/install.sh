...
echo "This script installs the 'fmt-center' utility and its man page."
echo
...
echo
echo "Building fmt-center..."
cd "$SCRIPT_DIR"
go build -o fmt-center ./main.go

echo "Installing binary to $INSTALL_DIR..."
mkdir -p "$INSTALL_DIR"
cp fmt-center "$INSTALL_DIR/fmt-center"
chmod +x "$INSTALL_DIR/fmt-center"

echo "Installing man page to $MAN_DIR..."
mkdir -p "$MAN_DIR"
cp fmt-center.1 "$MAN_DIR/fmt-center.1"

echo
echo "fmt-center installed successfully."
...
if command -v mandb >/dev/null 2>&1; then
  echo "Tip: run 'mandb' (possibly with sudo) if 'man fmt-center' is not immediately found."
fi
